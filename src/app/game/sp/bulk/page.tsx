...
const SinglePanelBulkPage = () => {
  ...
  const handleNumberClick = (digit: string) => {
    if (Number(amount) > 0 && Number(amount) >= 10) {
      const selectedPanaDigits = panaDigits[digit];
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <div className="justify-between flex items-center p-1">
            <input
              ...
              className=" rounded-lg shadow-lg p-2 max-w-[150px] text-center border-none focus:ring-transparent focus:outline-none"
            />
          </div>
        </div>

        <div className="grid grid-cols-5 mt-2 gap-2 justify-items-center">
          {Array.from({ length: 10 }, (_, digit) => {
            return (
              <div
                key={digit}
                className="relative bg-green-500 pt-2 pb-2 pl-6 pr-6 rounded-md text-white font-semibold text-center items-center"
